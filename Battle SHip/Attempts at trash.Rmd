...
## Then, create 10 X-variables using functions like rnorm(n, mean, sd), rchisq(n, df), rf(n, df1, df2), rt(n, df), rbeta(n, a, b), runif(n, a, b) or sample(c(1,0), n, replace=TRUE)... ## To see what any of these functions do, run codes like hist(rchisq(n, 3)). These functions are simply allowing you to get a random sample of x-values. But the way you choose your x-values can have quite an impact on what the final scatterplot of the data will look like.
 ...
 #True Model
    ...
    x9 <- runif(n,-4,4) #replace this
 
 
 #The Illusion
    x1 <- sample(c(0,1,2), n, replace=TRUE) #replace this
    x2 <- runif(n,-4,4)
    ...
    x4 <- runif(n,-0.1,0.1) #replace this
    x4 [x5==1] <- runif(n,-0.1,0.1)
    ...
    x6 <- x2*z1 #replace this
    x7 <- x2*x1#replace this
    
    x8 <- runif(n,-4,4)*-2 #replace this
    
    
    x10 <- (runif(n,-4,4)+1)*0.3 #replace this
...
## Then, create betas, sigma, normal error terms and y
 ...
 beta1 <- 0.30
    ...
    # beta7 * sin(beta8*x2) #SIN function??? not allowed...
 ...
 y <- beta0 + beta1*x9 + epsilon_i #...edit this code and replace it with your model. Don't forget the + epsilon_i!
 ...
 mylm <- lm(y ~ x9, data=rbdata) #edit this code to be your true model
 summary(mylm)
...
```{r}
mylm <- lm(y ~ x9, data=rbdata)
b <- coef(mylm)


plot(y~x9, data=rbdata, xlim=c(-4,4), ylim=c(-8,8))
curve(b[1] + b[2]*x, add=TRUE, col="red", lwd=2)
```
...
$$
  \text{EXAMPLE:} \ Y_i = \beta_0 + \beta_1 X_{9i} + \epsilon_i
$$
...
pairs(rbdata, panel = panel.smooth, col = interaction (rbdata$x1))

myattempt.lm <- lm(y~x4, data=rbdata)
summary(myattempt.lm)
...
mypred <- predict(mylm, data.frame(x=1), interval = "predict")
mypred
