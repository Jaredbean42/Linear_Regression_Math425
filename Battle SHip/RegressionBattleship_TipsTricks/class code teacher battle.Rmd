...
## Then, create 10 X-variables using functions like rnorm(n, mean, sd), rchisq(n, df), rf(n, df1, df2), rt(n, df), rbeta(n, a, b), runif(n, a, b) or sample(c(1,0), n, replace=TRUE)... ## To see what any of these functions do, run codes like hist(rchisq(n, 3)). These functions are simply allowing you to get a random sample of x-values. But the way you choose your x-values can have quite an impact on what the final scatterplot of the data will look like.
 ...
 #4  3 10  8  2  1  5  7  6  9

 x4 <- runif(n, -4, 4) #true x-axis
 x3 <- sample(c(0,1), n, replace=TRUE) #true switch
 x10 <- sample(c(0,1), n, replace=TRUE)
 
 x10[x3==1] <- 0
  
 x1 <- rt(n,2) #replace this
 x2 <- rf(n,5,1) #replace this
 x5 <- rbeta(n,3,18) #replace this
 x6 <- rbeta(n,0.1,0.1) #replace this
 x8 <- sample(c(0,1), n, replace=TRUE) #replace this
 x9 <- sample(c(0,1), n, replace=TRUE) #replace this
 x7 <- runif(n, -4, 4) + x9*5  #replace this
 
## Then, create betas, sigma, normal error terms and y
 
 # blue quadratic (baseline model)
 beta0 <- 4
 beta1 <- 1
 beta2 <- -0.5
 
 # changes needed for the red cubic
 beta3 <- -4 #change in intercept to get to 0.
 beta4 <- -3.3 #change in slope to get to -2.3
 beta5 <- 0.8 #change in quadratic to get to 0.3
 beta6 <- 0.2 #change in cubic to get to 0.2

 # changes needed for the green line
 beta7 <- -6
 beta8 <- 0.2
 beta9 <- 0.5
 ...
 y <- beta0 + beta1*x4 + beta2*x4^2 + #blue quadratic
      beta3*x3 + beta4*x4*x3 + beta5*x4^2*x3 + beta6*x4^3*x3 + #red cubic changes
   beta7*x10 + beta8*x4*x10 + beta9*x4^2*x10 + #green line changes
   + epsilon_i
 ...
 mylm <- lm(y ~ x4 + I(x4^2) + #blue quadratic
      x3 + x4:x3 + I(x4^2):x3 + I(x4^3):x3 + #red cubic
        x10 + x10:x4 + x10:I(x4^2) #green line
...
```{r, fig.height=5}
palette(c("steelblue","firebrick","green4"))
plot(y ~ x4, data=rbdata, col=interaction(x3,x10))

b <- coef(mylm)

x3=0
x10=0
curve(b[1] + b[2]*x4 + b[3]*x4^2 +
      b[4]*x3 + b[5]*x10 + b[6]*x4*x3 + b[7]*x4^2*x3 + b[8]*x3*x4^3 + 
      b[9]*x4*x10 + b[10]*x4^2*x10
      , add=TRUE, xname="x4", col="steelblue")

x3=1
x10=0
curve(b[1] + b[2]*x4 + b[3]*x4^2 +
        b[4]*x3 + b[5]*x10 + b[6]*x4*x3 + b[7]*x4^2*x3 + b[8]*x3*x4^3 + b[9]*x4*x10 + b[10]*x4^2*x10, add=TRUE, xname="x4", col="firebrick")

x3=0
x10=1
curve(b[1] + b[2]*x4 + b[3]*x4^2 +
        b[4]*x3 + b[5]*x10 + b[6]*x4*x3 + b[7]*x4^2*x3 + b[8]*x3*x4^3 + b[9]*x4*x10 + b[10]*x4^2*x10, add=TRUE, xname="x4", col="green4")
...
#Small JAred hint just have it match your b notice how similar it is to this just run b
 b
```
